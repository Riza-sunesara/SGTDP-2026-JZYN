...
|---|---|---|
| `NO_QUALIFYING_SCORE_THRESHOLD` | 0.35 | Below this, the top-ranked candidate's overall score is treated as too weak to recommend confidently (`no-qualifying-aed`) |
...
range/percentile here, so the 0.35 cutoff is backed by an observed number
rather than asserted as reasonable.]_
